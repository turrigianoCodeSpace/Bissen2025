function [s_objs, xPositions, yPositions, Label, RangeCut] = UnivarScatter(data, varargin)
%--------------------------------------------------------------------------
...
Ind=strmatch('WhiskerLineWidth',stringVars);
if ~isempty(Ind)
    %Check that LineWidth is a number
    if isnumeric(valueVars{Ind}) && all(size(valueVars{Ind})== [1,1])
        WishkerLineWidth=valueVars{Ind};
    else
        error(sprintf('wrong whisker LineWidth, it should be a positive number'))
    end
else
   WishkerLineWidth=1.5;
end
...
s_objs{i} = scatter(xValues,yValues,PointSize,PointStyle,'MarkerEdgeColor', MarkerEdgeColor(EdgeIndex,:),'MarkerFaceColor',MarkerFaceColor(FaceIndex,:),'LineWidth',LineWidth);

%If we want lines to represent the SEM and the std
if strcmp(Whiskers,'lines')
    %plot the mean line
    yMean=mean(yValues);
    plot([ix-Width/WhiskersWidthRatio ix+Width/WhiskersWidthRatio],[yMean yMean],'Color',MeanColor(MeanIndex),'LineWidth',WishkerLineWidth)
    
    %plot the sd 
    yStd=std(yValues);
    plot([ix-Width/WhiskersWidthRatio*0.5 ix+Width/WhiskersWidthRatio*0.5],[yMean+yStd yMean+yStd],'Color',StdColor(StdIndex,:),'LineWidth',WishkerLineWidth)
    plot([ix-Width/WhiskersWidthRatio*0.5 ix+Width/WhiskersWidthRatio*0.5],[yMean-yStd yMean-yStd],'Color',StdColor(StdIndex,:),'LineWidth',WishkerLineWidth)
    plot([ix ix],[yMean-yStd yMean+yStd],'Color',StdColor(StdIndex,:),'LineWidth',WishkerLineWidth)
    %plot the conf. interval of the mean
    ySem=yStd/sqrt(size(yValues,1));
    yCI=ySem*1.96;

    plot([ix-Width/WhiskersWidthRatio*0.7 ix+Width/WhiskersWidthRatio*0.7],[yMean+yCI yMean+yCI],'Color',SEMColor(SEMIndex,:),'LineWidth',WishkerLineWidth)
    plot([ix-Width/WhiskersWidthRatio*0.7 ix+Width/WhiskersWidthRatio*0.7],[yMean-yCI yMean-yCI],'Color',SEMColor(SEMIndex,:),'LineWidth',WishkerLineWidth)
    plot([ix ix],[yMean-yCI yMean+yCI],'Color',SEMColor(SEMIndex,:),'LineWidth',WishkerLineWidth)
